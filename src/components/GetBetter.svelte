...
<Modal>
  ...
  <form on:submit|preventDefault={handleSubmit}>
    ...
    <ul>
      <li
        class:active={rolled > 0 && rolled < 4}
        class:strike={rolled !== 0 && rolled > 3}
      >
        <b>1-3</b> Nothing
      </li>
      ...
      {#if rolled === 5}
        {#if equipment.length === 0}
          <li class="active">
            <button type="button" on:click={rollScroll("unclean")}
              >unclean scroll</button
            >
          </li>
        {:else}
      ...
      {#if rolled === 6}
        {#if equipment.length === 0}
          <li class="active">
            <button type="button" on:click={rollScroll("sacred")}
              >sacred scroll</button
            >
          </li>
        {:else}
          <li class="active">{equipment[0].name}</li>
