<script>
  ...
  const isEquippable = propSatisfies(
    (x) => !!find(equals(x), ["armor", "weapon"]),
    "type"
  );
  ...
  $: equipmentWeight = $character.equipment.reduce(
    (acc, eq) => acc + (!!eq.equipped === false ? eq.weight * eq.quantity : 0),
    0
  );
...
<div class="equipment" class:encumbered={isEncumbered}>
  <div class="row r">
    ...
    <p class="text">
      Strength + 8 ({encumbranceNumber}) items or DR+2 on Agility/Strength tests
    </p>
  </div>
  ...
  <ul class="list-clear list">
    {#each $character.equipment as e, i (e._id)}
      <li class="list-item">
        ...
        {/if}
        <!-- TODO: Skull Icon -->
        <button type="button" class="mb0" on:click={partial(dropEquipment, [i])}
          >Delete</button
        >
      </li>
    {/each}
